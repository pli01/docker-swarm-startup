...
# leader
echo "#leader $local_ip ok"
docker swarm init --listen-addr $local_ip:2377 --advertise-addr $local_ip
...
swarm_notify_success manager "$SWARM_MANAGER_TOKEN"
swarm_notify_success worker "$SWARM_WORKER_TOKEN"
# disable leader/manager as worker
docker node update --availability drain $(hostname)
docker node update --label-add leader=true $(hostname)
# manager
EOFMASTER
EOF
